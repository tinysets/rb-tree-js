...
export class RBTree {
    ...
    private _checkRbTree(root: RBNode, blackCount: number, currBlackCount: number): boolean {
    ...
    private isBlackOrNull(node: RBNode) {
        return node == null || node.color == RBColor.BLACK;
    }

    private isLeftNode(node: RBNode) {
        return node.parent.left == node;
    }

    private sibling(node: RBNode) {
        let p = node.parent
        if (p) {
            if (p.left == node) {
                return p.right;
    ...
    private _insert(root: RBNode, key: number) {
        while (true) {
            if (key < root.key) {
                if (root.left) {
                    root = root.left;
    ...
    private insert_blance(node: RBNode) {
    ...
    private _delete(root: RBNode, key: number) {
        while (root) {
            if (key < root.key) {
                root = root.left;
            } else if (key > root.key) {
    ...
    private delete_node(root: RBNode) {
    ...
    private delete_balance(x: RBNode, p: RBNode) {
        // return;
        if (x && x == this.root) {
